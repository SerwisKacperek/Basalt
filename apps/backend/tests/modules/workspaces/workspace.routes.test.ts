...
function makeApp(overrides: Record<string, any> = {}) {
  ...
  return {
    app: createCrudRoutes('/workspaces', controller as any, WorkspaceBody, WorkspaceResponse),
    controller,
  };
}
...
describe('workspace routes', () => {
  ...
  describe('POST /workspaces', () => {
    ...
    it('returns 422 when name is empty string', async () => {
      const { app } = makeApp();
      expect((await post(app, '/workspaces', { name: '' })).status).toBe(422);
    });
  ...
  describe('PATCH /workspaces/:id', () => {
    ...
    it('returns 422 when name is empty string', async () => {
      const { app } = makeApp();
      expect((await patch(app, '/workspaces/1', { name: '' })).status).toBe(422);
    });
  });
